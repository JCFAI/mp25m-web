-- Incremento 7C.2
-- Gobernanza de evaluación humana de matches y cobertura de requerimientos.
--
-- Principios:
-- - sólo la revisión actual, activa y validada admite nuevas evaluaciones;
-- - sólo un match accepted_for_analysis admite nuevas evaluaciones;
-- - las evaluaciones son append-only;
-- - una reevaluación crea una nueva versión;
-- - stale writes se rechazan mediante expected assessment/evaluation number;
-- - un candidato no resuelto sólo admite insufficient_evidence;
-- - una evaluación sustantiva de match debe apoyarse en foundations congeladas;
-- - una cobertura nueva sólo utiliza evaluaciones actuales de matches;
-- - covered, partial y missing son conclusiones humanas gobernadas;
...
-- ===========================================================================
-- 1. AUTORIZACIÓN DE EVALUACIÓN
-- ===========================================================================
--
-- En 7C usamos inicialmente la misma autoridad de escritura que 7B.2:
-- admin, validator, articulator o responsable actual de la oportunidad.
--
-- Se encapsula en un helper propio para no acoplar semánticamente 7C
-- a la operación "decide" de 7B.
...
is
  '7C authorization helper. Initial write authority mirrors governed 7B match decisions while keeping evaluation semantics separate.';


-- ===========================================================================
-- 2. EVALUAR / REEVALUAR MATCH
-- ===========================================================================

create function
mp25m_api.assess_opportunity_requirement_match(
  p_actor_internal_user_id uuid,
  p_match_id uuid,
  p_expected_assessment_no integer,
  p_assessment_kind text,
  p_rationale text,
  p_foundation_ids uuid[] default '{}'::uuid[]
)
returns table (
  assessment_id uuid,
...
is
  'Creates the next immutable human assessment of an accepted match, with stale-write protection and exact frozen foundation selection. Unresolved candidates may only receive insufficient_evidence.';


-- ===========================================================================
-- 3. EVALUAR / REEVALUAR COBERTURA
-- ===========================================================================
...
is
  'Creates the next immutable human coverage conclusion for an exact current validated requirement revision, using only current assessments of accepted matches and stale-write protection.';
