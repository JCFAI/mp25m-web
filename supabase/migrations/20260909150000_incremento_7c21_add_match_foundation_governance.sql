-- Incremento 7C.2.1
-- Agregado gobernado de evidencia existente a un match analítico.
--
-- Principios:
-- - no modifica matches, assessments ni evaluaciones históricas;
-- - sólo opera sobre el match actual, aceptado para análisis;
-- - el cliente identifica una evidencia existente, pero no declara su
--   contenido observado como verdad;
-- - el servidor relee actor_search_evidence y congela el snapshot;
-- - la evidencia debe pertenecer exactamente al actor del match;
-- - direct exige igualdad canónica estructurada con skill/activity;
-- - candidatos no resueltos sólo pueden aportar contexto;
-- - la misma evidencia no se agrega dos veces al mismo match;
-- - foundation no implica satisfacción ni cobertura.
...
is
  'Adds one existing server-reread actor evidence record as an immutable added_evidence foundation of a current accepted analytical match. Direct relation requires exact canonical skill/activity equality; unresolved candidates remain contextual.';
