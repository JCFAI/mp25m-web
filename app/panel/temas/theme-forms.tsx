...
import {
  ...
  createThemeAction,
  createThemeFollowupAction,
  removeThemeResponsibilityAction,
  transitionThemeAction,
  updateThemeAction,
  type ThemeActionState,
} from './actions'

const initialState: ThemeActionState = { status: 'idle', message: null }
const priorities = { low: 'Baja', normal: 'Normal', high: 'Alta', urgent: 'Urgente' }
const statuses = { active: 'Activo', monitoring: 'En seguimiento', paused: 'Pausado', closed: 'Cerrado' }
const responsibilityRoles = { principal: 'Principal', responsible: 'Responsable', promoter: 'Impulsor' }
const followupTypes = { general: 'Novedad general', meeting: 'Reunión', decision: 'Decisión', commitment: 'Compromiso', next_step: 'Próximo paso', result: 'Resultado', observation: 'Observación' }

function Feedback({ state }: { state: ThemeActionState }) {
...
export function ThemeGovernanceForms({ theme, canManage, canGovern }: { theme: Theme; canManage: boolean; canGovern: boolean }) {
  const [updateState, updateAction, updatePending] = useActionState(updateThemeAction.bind(null, theme.theme_id), initialState)
  const [transitionState, transitionAction, transitionPending] = useActionState(transitionThemeAction.bind(null, theme.theme_id), initialState)
  return <div className="space-y-5">
    {canManage ? <details className="rounded-2xl border border-slate-200 bg-white shadow-sm"><summary className="cursor-pointer px-5 py-4 text-sm font-semibold text-[#1E3A5F]">Editar datos del tema</summary><form action={updateAction} className="grid gap-4 border-t border-slate-200 p-5 sm:grid-cols-2"><label className="text-sm font-semibold text-slate-700 sm:col-span-2">Nombre<input name="name" required minLength={3} maxLength={200} defaultValue={theme.name} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700 sm:col-span-2">Descripción<textarea name="description" required minLength={10} maxLength={10000} defaultValue={theme.description} rows={3} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700 sm:col-span-2">Propósito<textarea name="purpose" required minLength={3} maxLength={10000} defaultValue={theme.purpose} rows={3} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700">Prioridad<select name="priority" defaultValue={theme.priority} className={inputClass}>{Object.entries(priorities).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-sm font-semibold text-slate-700">Inicio<input name="start_date" type="date" required defaultValue={theme.start_date} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700 sm:col-span-2">Situación actual<textarea name="current_summary" minLength={3} maxLength={10000} defaultValue={theme.current_summary ?? ''} rows={3} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700 sm:col-span-2">Alcance territorial<textarea name="territorial_scope_summary" minLength={3} maxLength={2000} defaultValue={theme.territorial_scope_summary ?? ''} rows={2} className={inputClass} /></label><label className="text-sm font-semibold text-slate-700 sm:col-span-2">Fundamento del cambio<textarea name="rationale" required minLength={3} maxLength={10000} rows={2} className={inputClass} /></label><div className="sm:col-span-2"><Feedback state={updateState} /><button disabled={updatePending} className="mt-3 rounded-xl bg-[#1E3A5F] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{updatePending ? 'Guardando...' : 'Guardar cambios'}</button></div></form></details> : null}
    {canGovern ? <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-950">Estado</h2><form action={transitionAction} className="mt-4"><select name="status" defaultValue={theme.status} className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm">{Object.entries(statuses).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><textarea name="rationale" required minLength={3} maxLength={10000} rows={2} placeholder="Motivo del cambio..." className={`${inputClass} mt-3`} /><textarea name="closing_summary" minLength={3} maxLength={10000} rows={2} placeholder="Síntesis final (obligatoria al cerrar)..." className={`${inputClass} mt-3`} /><Feedback state={transitionState} /><button disabled={transitionPending} className="mt-3 rounded-xl border border-[#2F5D8C] px-4 py-2.5 text-sm font-semibold text-[#1E3A5F] disabled:opacity-50">{transitionPending ? 'Actualizando...' : 'Actualizar estado'}</button></form></section> : null}
  </div>
}
...
function ThemeResponsibilityCard({ themeId, responsibility, canGovern }: { themeId: string; responsibility: ThemeResponsibility; canGovern: boolean }) {
  const [state, action, pending] = useActionState(removeThemeResponsibilityAction.bind(null, themeId, responsibility.responsibility_id), initialState)
